...
Current production-facing portal support in the factory is CKAN, DCAT udata REST, SPARQL-backed DCAT, static Project Open Data `data.json`, Socrata, OpenDataSoft, ArcGIS Hub, OGC CSW, STAC Collections, and SDMX dataflows. For ad-hoc SPARQL DCAT harvests, use `--type dcat --profile sparql`; for config-driven harvests, set `profile = "sparql"` and optionally `sparql_endpoint`. SPARQL full harvests use publisher-bounded pages (or a keyset cursor for `data.europa.eu`), then separate bounded `VALUES` phases for dataset fields and distributions; the latter persists inline resource title, format, media type, and download/access URLs without multiplying the pagination query. ArcGIS Hub sites whose injected `catalogV2` has an empty item scope are rejected because their search endpoint returns global ArcGIS content instead of datasets belonging to that portal. STAC harvesting follows landing-page and pagination links but never follows Collection `items` links. CSW harvesting narrows a failing window (100 → 25 → 5 → 1) and skips a single unreadable record rather than abandoning the rest of the catalogue. Any such skip is emitted as a terminal partial-harvest diagnostic after later records have been preserved, so stale marking and incremental-sync advancement stay disabled and the CLI exits 2. It asks for the ISO 19115 profile and falls back per catalogue to the mandatory Dublin Core profile on an `InvalidParameterValue` rejection; the choice is memoized, and because OWS reports it as HTTP 400 the failing response body is read rather than discarded. SDMX harvesting reads the whole dataflow structure message in one bounded request (the standard defines no pagination for structure queries) and emits it in 500-row chunks; it never queries observation data, and every sync is a full sync.
