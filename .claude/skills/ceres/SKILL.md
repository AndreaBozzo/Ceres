...
- The multi-stage image contains `ceres`, `ceres-server`, bundled migrations, and `ceres-migrate`. It defaults to the server; schedulers override the command with `ceres harvest --metadata-only` and mount `PORTALS_CONFIG` read-only.
